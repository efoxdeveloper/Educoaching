...
export function AddCourseDrawer({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not add course. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Drawer open={open} onClose={onClose} title="Create Course & Program">
      <form onSubmit={handleSubmit} className="space-y-4 pb-4">
        ...
        <Field label="Target Exam / Stream">
          <input
            ...
            placeholder="e.g. NEET-UG, JEE, CBSE 10, or Crash Course"
        ...
        <div className="rounded-xl border border-scholar-200 bg-scholar-50/50 p-3.5 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            >
              Full Course Fee
              <span className="block text-[10px] opacity-80">(One-time / Installments)</span>
            </button>
          ...
          <Field
            label={`Fee Amount (₹) ${
              form.feeType === "ONE_TIME"
                ? "— Full Program Total (Can be collected in Installments)"
                : form.feeType === "MONTHLY"
                ? "— Per Month"
                : form.feeType === "QUARTERLY"
        ...
        {/* Branch Allocation / Multi-Branch Delivery */}
        ...
        <Field label="Course Details, Syllabus & Inclusions (Optional)">
          <textarea
            ...
            placeholder="e.g. Complete NCERT + Competitive Modules, 40+ Full-length CBT Mock Tests, Weekly Doubt Clearing & Mentorship Sessions."
          />
        </Field>
